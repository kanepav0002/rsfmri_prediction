...
if outputFlag=="aGMR":
	...
	dbscan_tsv=folder+subject+"_dbscan"


output=folder+nifti_gz[0:len(nifti_gz)-7]+"_"+outputFlag+"_"
...
iterations = total2.shape[1]
for k in range(iterations):
    k=k+1
    n_reg = str(k)
    file_name_tsv=dbscan_tsv+"_"+n_reg+"regressors.tsv" 
    np.savetxt(file_name_tsv, total2[:,0:k], delimiter="\t")
...
for k in range(iterations):
    k=k+1
    out_file = output+str(k)+"regressors.nii.gz"
    db_design=dbscan_tsv+"_"+str(k)+"regressors.tsv"
    call_fslregfilt=["fsl_regfilt","-i",nifti_gz,"-o",out_file,"-d",db_design,"-v","-f",filtString[0:filt_count]]
