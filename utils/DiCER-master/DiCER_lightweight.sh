...
while getopts 'i:t:a:w:s:c:m:dp:r:e:fh' flag; do
  case "${flag}" in
    i)  input_file="${OPTARG}" ;;  
    t)  tissue="${OPTARG}" ;;  
	a)  anatomical="${OPTARG}" 
...
python carpetCleaning/clusterCorrect.py $tissue_mask '.' $input $folder $subject $iterations $epsilon
...
python carpetCleaning/vacuum_dbscan_multi_save.py -f $input_file -db $regressor_dbscan -s $subject -d $folder"/"
...
# Move dbscan files from input directory to output directory
#base_input=`basename $input_file .nii.gz`
#dir_input=$(dirname $input_file)
#mv $output_folder/$dir_input/$base_input"_"* $output_folder/
